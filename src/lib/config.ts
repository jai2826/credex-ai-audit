...
export const TOOLS_CONFIG: Record<string, ToolConfigData> =
  Object.entries(TOOL_REGISTRY).reduce(
    (acc, [uiKey, mapping]) => {
      const saasData = mapping.saas
        ? SAAS_PRICING_DB[mapping.saas]
        : null;
      const apiData = mapping.api
        ? API_PRICING_DB[mapping.api]
        : null;
      ...
      acc[uiKey] = {
        name:
          mapping.displayName ||
          saasData?.name ||
          uiKey.replace("_", " ").toUpperCase(),
        supportsSaaS: !!saasData,
        supportsAPI: !!apiData,
        apiProviderKey: mapping.api,
        plans: saasData
          ? saasData.plans.map((p) => p.name)
          : [],
        models: apiData
          ? apiData.map((m) => ({
              id: m.modelId,
...
// export const API_MODEL_OPTIONS = Object.entries(
//   TOOLS_CONFIG,
// ).reduce(
//   (acc, [uiKey, config]) => {
//     if (config.supportsAPI && config.apiProviderKey) {
//       acc[uiKey] = config.models;
//     }
//     return acc;
//   },
